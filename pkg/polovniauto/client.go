...
func (c *Client) GetCarsList(ctx context.Context) (map[string][]string, error) {
	allocCtx, cancel := chromedp.NewRemoteAllocator(ctx, c.cfg.ChromeWSURL)
	defer cancel()

	// create timeout
	ctx, cancel = context.WithTimeout(allocCtx, c.cfg.ChromeTimeout)
	defer cancel()
