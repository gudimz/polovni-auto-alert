...
func run() {
	...
	jobsList := map[string]func(context.Context) error{
		...
		job.NotifierRegionsJobName:  svc.UpdateRegionsList,
		job.NotifierChassisJobName:  svc.UpdateChassisList,
	}
	...
	for name, f := range jobsList {
		...
		go func(name string, f func(context.Context) error) {
			...
			if err = f(ctx); err != nil {
				l.Error("failed to run critical job before start", logger.ErrAttr(err), logger.StringAttr("job_name", name))
				errCh <- err
			}
		}(name, f)
	}
